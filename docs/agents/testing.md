...
`python3 hack/jira-render-evidence.py verify` replays every archive's `input.jira` against the live renderer and diffs the stored `rendered.html`, so a new archive needs no entry anywhere else — its own `input.jira` is the probe. Capture new evidence with `python3 hack/jira-render-evidence.py probe '<markup>' --json`; the probe campaigns in that script are a frozen historical archive that fixtures and test comments cite by name, and `python3 hack/jira-render-evidence.py replay --list` enumerates them.
