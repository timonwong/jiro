...
JFM therefore distinguishes a Jira Emoticon from literal text. The canonical representation is the attrless inline directive `:emoticon[token]`, where `token` is exactly one token already verified by the shared Jira inline grammar. The directive has no attributes, does not accept nested inline content, and does not accept unknown tokens. The renderer-backed input aliases canonicalize pairwise: `(*y)` to `(*)`, `:p` to `:P`, `:-)` to `:)`, `:-(` to `:(`, and `;-)` to `;)`; no other token aliases are introduced without renderer evidence.
